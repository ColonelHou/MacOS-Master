package com.java.algorithm.recommand.simi;
 ...
 * 1. 计算出用户相似度:
 *    a. 输入一个目标用户, 在用户列表中匹配
 *    b. 迭代用户点评电影集, 计算出相似度(差、平方、和、根号)
 *    c. 排序取出top2相似度用户
 *
 * 2. 计算电影推荐度
 *    a. 从相似的两个用户电影评分中, 用各自的相似度 * 评分 相加 与 7部平均值比较, 大的拿出来要推荐
 */
public class UserCFDemo {
    ...
    private static void calcRecommendMovie() {
        ...
        // 每一评分 与前两个相似用户的 相似度 相乘
        for (int i = 0; i < 7; i++) {
            recommendMovie = new ArrayList<>();
            recommendMovie.add(i);
            recommdRate = allUserMovieStarList[
                    Integer.parseInt(similarityUsers.get(0).get(0).toString())// 第一个相似用户中 用户下标
                    ][i] // 评分
                    *
                    Double.parseDouble(similarityUsers.get(0).get(1).toString()) // 第一个相似用户中 与目标相似度

                    + allUserMovieStarList[
                            Integer.parseInt(similarityUsers.get(1).get(0).toString()) // 用户id
                    ][i] *
                    Double.parseDouble(similarityUsers.get(1).get(1).toString());  // 相似度
